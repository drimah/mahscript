console.log("mahscript v7.0 - Capa Full Page & UI Corrigida");

// ========================================================================
// 1. CONFIGURAÇÃO DE TEMAS (Originais + 6 Novos)
// ========================================================================
const mahThemes = {
    // Temas Originais
...
// ========================================================================
// 2. FUNÇÕES DE UI E ESTILO (CSS Dinâmico)
// ========================================================================
...
/* CORREÇÃO CRÍTICA: INPUTS COM TEXTO PRETO E FUNDO BRANCO */
/* Usando seletor específico para garantir override no painel escuro */
...
#mahscript-settings-section .mah-form select { 
    ...
    -webkit-text-fill-color: #000000 !important; /* Força cor no Webkit */
    font-size: 13px !important; box-sizing: border-box !important; outline: none !important; 
    font-family: Arial, Roboto, sans-serif !important; transition: border-color 0.2s !important; 
}
...
/* FORÇA TEXTO BRANCO EM TODO O CONTEÚDO DO PAINEL, MAS PERMITE INPUTS ESCUROS */
#mahscript-docs-chapter-panel *:not(input):not(textarea):not(select) {
color: #ffffff !important;
...
// FUNÇÃO ATUALIZADA: TOOLTIP APENAS COM NOME (SEM "TEMA:")
...
Object.entries(mahThemes).forEach(([key, theme]) => {
    ...
    // Apenas o nome do tema, sem prefixo
...
// HTML atualizado: BOTÃO SALVAR REMOVIDO
container.innerHTML = `
...
// SALVAMENTO AUTOMÁTICO AO DIGITAR (SUBSTITUI O BOTÃO SALVAR)
...
// ========================================================================
// FUNÇÃO REESCRITA: CAPA EM TAMANHO PÁGINA INTEIRA (A4)
// ========================================================================
function insertCoverImage(imageData) {
    const editor = document.querySelector('.kix-appview-editor');
    if (!editor) return;
    
    // Foca no editor e posiciona o cursor no início absoluto
    ...
    setTimeout(() => {
        try {
            // Posiciona o cursor no primeiro caractere do documento
            ...
            // AGORA DEFINE O TAMANHO DA IMAGEM PARA PÁGINA INTEIRA (A4)
            // Aguarda um pouco para a imagem ser renderizada no DOM
            setTimeout(() => {
                const insertedImg = editor.querySelector('img[src^="data:image"]');
                if (insertedImg) {
                    // Define tamanho A4 em pontos (595x842)
...
this.panel.innerHTML = `<div class="mahscript-chapter-header-row"><div class="mahscript-chapter-header-title">Capítulos</div><button class="mahscript-chapter-add" id="mahscript-add-chapter" title="Adicionar guia">+</button></div><div id="mahscript-inline-prompt" style="display: none;"><input type="text" id="mahscript-prompt-input" placeholder="Nome da guia..."><div class="mahscript-prompt-actions"><button id="mahscript-prompt-cancel" class="mahscript-prompt-btn">Cancelar</button><button id="mahscript-prompt-ok" class="mahscript-prompt-btn">OK</button></div></div><div id="mahscript-chapters-area" style="display: none;"><div id="mahscript-chapter-list"></div></div><div id="mahscript-settings-section" style="display: none;"></div>`;
